...
plt.plot(
    alpha_vals * 180 / np.pi,
    CL_fit,
    "b-",
    label=f"CL fit: {popt_CL[0]:.4f}α² + {popt_CL[1]:.4f}α + {popt_CL[2]:.4f}",
)
...
plt.plot(
    ...
    CD_fit,
    "r-",
    label=f"CD fit: {popt_CD[0]:.4f}α² + {popt_CD[1]:.4f}α + {popt_CD[2]:.4f}",
)
...
plt.plot(alpha_vals * 180 / np.pi, CL_vals**3 / (CD_vals + 0.04) ** 2)
plt.show()
