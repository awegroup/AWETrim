...
chi_func = ca.Function(
    "chi", [kinematics.t, kinematics.s, kinematics.s_dot,rh], [kinematics.chi]
)

vk_func = ca.Function(
    "vk", [kinematics.t, kinematics.s, kinematics.s_dot,rh], [kinematics.vk]
)

vr_func = ca.Function("vr", [kinematics.t], [kinematics.vr])
dot_chi_func = ca.Function(
    "dot_chi", [kinematics.t, kinematics.s, kinematics.s_dot,rh], [kinematics.dot_chi]
)
vtau_func = ca.Function(
    "vtau", [kinematics.t, kinematics.s, kinematics.s_dot,rh], [kinematics.vtau]
)
...
state.timeder_angle_course =  dot_chi_func(time_sym, s_sym, s_dot_sym,rh)
state.speed_tangential = vtau_func(time_sym, s_sym, s_dot_sym,rh)
state.angle_course = chi_func(time_sym, s_sym, s_dot_sym,rh)
...
si = ca.MX.sym("si")
t = ca.MX.sym("t")
tf = (sf-si)/s_dot_sym + t
t_func = ca.Function("t_func", [si,sf,t,s_dot_sym], [tf])
...
tension_tether = 0
residual = ca.Function("residual", [time_sym,s_sym,s_dot_sym,rh, state.input_steering, state.length_tether], [state.residual])
for i in range(len(s)-1):
    opti.subject_to(residual(time_var[i],s[i],s_dot_var[i],rh_var,input_steering_var[i],length_tether_var[i]) == 0)
    opti.subject_to(time_var[i+1] == t_func(s[i],s[i+1],time_var[i],s_dot_var[i]))
    tension_tether += tension_tether_func(pattern.r(time_var[i]),length_tether_var[i])


opti.subject_to(5 >= (input_steering_var[:] >= -5))
opti.subject_to(300 >= (length_tether_var[:] >= 195))
# opti.subject_to(0 <= (s_var[:] <= 8*np.pi))
opti.subject_to(60 <= (rh_var <= 120))

opti.set_initial(rh_var, 90)
