...
def create_model_and_phase(kite, tether, pattern_config, start_state):
    model = SystemModel(dof=3, quasi_steady=True, kite=kite, wind_model="logarithmic", tether=tether)
    model.wind.speed_friction = 2
...
def plot_results(phases):
    ...
    for i, ((ry, rz, ky, kz), phase) in enumerate(phases.items()):
        ...
        ax.scatter(azimuth, elevation, c=vtau, cmap="viridis", s=10, label=f"ky={ky}, kz={kz}")
...
def simulate_parameter_grid():
    ...
    start_state = State(t=0, s=np.pi/2, s_dot=2, s_ddot=0, length_tether=200,
                        input_steering=0, angle_roll=0, angle_pitch=0, angle_yaw=0)
    ...
    for ry, rz, ky, kz in itertools.product(ry_vals, rz_vals, ky_vals, kz_vals):
        pattern_config = {
            ...
            "parameters": {
                "omega": -1.0, "r0": 200.0, "ry": ry, "rz": rz,
                "ky": ky, "kz": kz, "vr": 1, "beta0": 0.45, "kappa": 0
            },
