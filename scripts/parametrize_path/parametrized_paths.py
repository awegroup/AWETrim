...
from picawe.system.tether import FlexibleLinkTether, RigidLumpedTether, FlexibleLumpedTether
...
for i,mr in enumerate(mass_ratio_values):
    for quasi_steady in [True,False]:  # Loop over both dynamic and quasi-steady cases
        ...
        kite_model.wind.speed_wind_ref = 18
        kite_model.input_depower = 0
        kite_model.speed_radial = 0
        # kite_model.angle_pitch_tether = tether_angles[i]
...
for i, mr in enumerate(mass_ratio_values):
    ...
    tension_qs = phase_qs.return_variable("length_tether")[mask_qs]
    tension_dyn = phase_dyn.return_variable("length_tether")[mask_dyn]
