...
for i, (ry, rz, ky, kz) in enumerate(itertools.product(ry_vals, rz_vals, ky_vals, kz_vals)):
    figure = FigureEight(omega, r0, ry, rz, vr, beta_p,ky, kz)
    beta = figure.elevation(t, s)
    azimuth = figure.azimuth(t, s)
    ax.plot(np.degrees(azimuth), np.degrees(beta), label=f"ry={ry}, rz={rz}, ky={ky}, kz={kz}", color=colors[i % len(colors)])
    # plt.show()
...
for i, (ry, rz, ky, kz) in enumerate(itertools.product(ry_vals, rz_vals, ky_vals, kz_vals)):
    ...
    y = figure.y(t, s)
    z = figure.z(t, s)
    ax.plot(y, z, label=f"ry={ry}, rz={rz}, ky={ky}, kz={kz}", color=colors[i % len(colors)])
ax.set_xlabel("y [m]")
ax.set_ylabel("z [m]")
# ax.legend()
