...
kite2 = Kite(mass_wing=43, area_wing=20, aero_input=aero_input, mass_kcu=0, steering_control="asymmetric")
...
qs_guess = [1e2, 0, 40]
qs_guess2 = [1e2, 0, 40]
flight_data.kite_azimuth = flight_data.kite_azimuth-0.18
...
solve_func2, inputs_name2 = kite_model2.solve_quasi_steady_state(
        unknown_vars, solver_options=solver_options
    )
...
for i, row in flight_data.iterrows():
    ...
    # print(p)
    lbx,ubx,lbg,ubg = kite_model2.get_boundaries(unknown_vars)
    # print(lbx,ubx,lbg,ubg)
...
print("Mean CL, exp. data: ", np.mean(results["wing_lift_coefficient"]))
print("Mean CD, exp. data: ", np.mean(results["wing_drag_coefficient"]))
print('Mean CL,  kcu: ', np.mean(solutions_df['lift_coefficient']))
print('Mean CD,  kcu: ', np.mean(solutions_df['drag_coefficient']))
print('Mean CL,  no kcu: ', np.mean(solutions_df2['lift_coefficient']))
print('Mean CD,  no kcu: ', np.mean(solutions_df2['drag_coefficient']))


print("Mean aoa, exp. data: ", np.mean(results["wing_angle_of_attack_bridle"]))
print('Mean aoa,  kcu: ', np.mean(solutions_df['angle_of_attack'])*180/np.pi)

import matplotlib.pyplot as plt

plt.figure()
plt.plot(speed_tangential, label="Measured speed_tangential")
plt.plot(solutions_df["speed_tangential"], label="Estimated speed_tangential, KCU")
plt.plot(solutions_df2["speed_tangential"], label="Estimated speed_tangential, no KCU")
plt.legend()

plt.figure()
plt.plot(flight_data["ground_tether_force"], label="Measured tension_tether_ground")
plt.plot(solutions_df["tension_tether_ground"], label="estimated tension_tether, KCU")
plt.plot(solutions_df2["tension_tether_ground"], label="estimated tension_tether, no KCU")
plt.legend()
...
# PLot steering input
plt.figure()
plt.plot(flight_data["kcu_actual_steering"]/100, label="us measured")
plt.plot(solutions_df["input_steering"], label="u_s")
plt.plot(solutions_df2["input_steering"], label="u_s no KCU")
plt.legend()
# plt.show()
...
# plt.legend()
# plt.show()
mask = (flight_data.kite_elevation < 0.8)
total_power = (
...
print("Estimated power KCU: ", total_power, "W")
print("Estimated power no KCU: ", total_power2, "W")
...
ax3.plot(flight_data["time"], solutions_df["speed_tangential"], label="QS $v_{\\tau}$, KCU")
...
ax4.plot(flight_data["time"], solutions_df["tension_tether_ground"], label="QS $F_{t,g}$, KCU")
...
ax5.plot(flight_data["time"], solutions_df["input_steering"], label="QS $u_s$, KCU")
