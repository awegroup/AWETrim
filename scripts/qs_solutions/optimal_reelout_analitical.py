...
def define_force_function(C_L: float, C_D: float, phi: float = 0.0, beta: float = 0.0,
                          rho: float = 1.225, S: float = 46.0) -> ca.Function:
    """
...
def compute_optimal_reel_speeds_and_forces(
    ...
    for lift_coefficient, drag_coefficient in aero_coefficients:
        force_function = define_force_function(lift_coefficient, drag_coefficient, phi, beta)
        v_r = ca.SX.sym('v_r')
        v_w = ca.SX.sym('v_w')
        ...
        average_energy_expr = ((power_expr * reel_out_length / v_r) + reel_in_energy) / (
        ...
        for wind_speed in wind_speeds:
            ...
            try:
                ...
                corresponding_tether_forces.append(float(force_function(optimal_speed, wind_speed)))
            except Exception as e:
                logger.warning(f"Root solver failed for wind_speed={wind_speed:.2f} m/s: {e}")
...
def analytical_solution(wind_speeds, beta=0.0, phi=0.0, rho=1.225, S=46.0, C_L=0.75, C_D=0.15):
    """
...
def plot_results(results, wind_speeds, vr_opt, ft_opt):
    ...
    plt.plot(results[pairs[0]]['v_r_sol'], results[pairs[0]]['ft_sol'] / 9.81,
             label='With reelin')

    plt.fill_between(results[pairs[1]]['v_r_sol'],
                     results[pairs[1]]['ft_sol'] / 9.81,
                     results[pairs[2]]['ft_sol'] / 9.81,
                     alpha=0.5, label='With reelin (min-max aero performance)')

    plt.plot(vr_opt, ft_opt / 9.81, label='Without reelin (analytical-optimal)')
    plt.plot([-0.2, 1.4, 10], [440, 2260, 2890], linestyle="--", label="Current curve KP")
...
def main():
    ...
    results, wind_speeds = compute_optimal_reel_speeds_and_forces(cl_cd_pairs, deltaL, F_reelin, v_r_reelin)
