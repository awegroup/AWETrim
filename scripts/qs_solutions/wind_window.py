...
phi_values = np.radians(np.linspace(-60, 60, 30))  # Range for phi in radians
beta_values = np.radians(np.linspace(0, 80, 30))  # Range for beta in radians
...
for phi, beta in zip(phi_combinations, beta_combinations):
    ...
    current_state = {
        ...
        "angle_azimuth": phi,
        "speed_wind_ref": kite_model.speed_wind_ref,
    }
