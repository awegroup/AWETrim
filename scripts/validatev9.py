...
mask = flight_data.cycle.isin(range(15,16))
...
# Example Usage
kite = Kite(mass_wing=50, area_wing=47, aero_input=aero_input, mass_kcu=28, steering_control="asymmetric")
kite_model = SystemModel(dof=3, quasi_steady=True, kite=kite, tether=tether, wind_model=wind_model)
kite2 = Kite(mass_wing=78, area_wing=47, aero_input=aero_input, mass_kcu=0, steering_control="roll")
...
if "kite_yaw_rate_1" not in flight_data.columns:
    flight_data["kite_yaw_rate_1"] = np.gradient(flight_data["kite_course"], flight_data["time"])
print(solve_func)
vtau = []
vtau2 = []
...
for i, row in flight_data.iterrows():
    ...
    uf = row["100m_Wind_Speed_m_s"]*kite_model.wind.kappa/np.log(100/kite_model.wind.z0) 
...
ax5.plot(flight_data["time"], flight_data["kcu_actual_steering"]/max(flight_data["kcu_actual_steering"]), label="Meas. $u_s$")
ax5.plot(solutions_df["time"], solutions_df["input_steering"]/max(solutions_df["input_steering"]), label="QS $u_s$, KCU")
ax5.plot(flight_data["time"], solutions_df2["input_steering"]/max(solutions_df2["input_steering"]), label="QS $u_s$, no KCU")
ax5.set_ylim(-2, 2)
