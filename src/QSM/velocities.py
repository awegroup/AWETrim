...
def calculate_tangential_speed(azimuth, elevation,wind_speed, kite_speed, heading = 0):
    """
    Calculate the tangential speed of the kite.
    """
    return kite_speed + wind_speed*( -np.sin(azimuth)*np.sin(heading)-np.sin(elevation)*np.cos(heading)*np.cos(heading))
...
def calculate_apparent_speed(azimuth, elevation, reelout_speed, wind_speed, kite_speed, heading = 0):
    ...
    tangential_speed = calculate_tangential_speed(azimuth, elevation,wind_speed, kite_speed, heading)
...
def calculate_kite_speed(aoa_trim,azimuth, elevation, reelout_speed, wind_speed, heading =0):
    ...
    """
    radial_speed = calculate_radial_speed(azimuth, elevation, reelout_speed, wind_speed)
    kite_speed = radial_speed/ np.tan(aoa_trim) - wind_speed*( -np.sin(azimuth)*np.sin(heading)-np.sin(elevation)*np.cos(heading)*np.cos(heading))
    return kite_speed
