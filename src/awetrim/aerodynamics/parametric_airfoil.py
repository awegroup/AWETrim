...
"""Parametric 2D Leading-Edge-Inflatable (LEI) airfoil-section generator.

This module owns a small, dependency-light (numpy only) parametric description
of a soft-kite **airfoil section**. It is the 2D-section counterpart to the
3D-planform generator in :mod:`awetrim.aerodynamics.parametric_geometry`: both
are pure-geometry generators with no VSM/CasADi dependency that feed the VSM
trim/sweep path.

The profile is built from cubic Bezier curves controlled by six design
parameters (tube size ``t``, max-camber position ``eta`` and height ``kappa``,
trailing-edge reflex ``delta``, camber tension ``lambda`` and leading-edge
tension ``phi``). The parametrisation follows the regression model of Masure for
LEI kite airfoils.

Plotting lives separately in
:func:`awetrim.plotting.plotting.plot_lei_airfoil` so this module stays pure.
"""
...
def save_profile_as_dat_file(all_points, profile_name, output_file_path, seam_a=None):
    ...
    if seam_a is not None:
        logger.info("Seam angle: %.1f deg", np.degrees(seam_a))
