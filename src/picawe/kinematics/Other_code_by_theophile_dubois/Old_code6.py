...
class ReelInBspline():
    ...
    def __init__(self, file_path_full, file_path_cycle, cycle_idx=0):
        ...
        self.ri_p0, self.ri_pf = None, None
        self.ri_v0, self.ri_vf = None, None
        self.az_ri, self.el_ri, self.r_ri = None, None, None
        self.x_ri, self.y_ri, self.z_ri = None, None, None
        self.ri_c0, self.ri_cf = None, None
        self.ri_idx0, self.ri_idxf = None, None
    ...
    def _compute_cycle_indices(self):
        ...
        self.cycle_idx0 = self.start_indices[self.cycle_idx]
        self.cycle_idxf   = self.start_indices[self.cycle_idx + 1] - 1 if self.cycle_idx + 1 < len(self.start_indices) else len(self.time_full)-1

    def _extract_cycle_data(self):
        """Extract only the selected cycle data (spherical and cartesian)."""
        self.az_cyc = self.az[self.cycle_idx0:self.cycle_idxf+1]
        self.el_cyc = self.el[self.cycle_idx0:self.cycle_idxf+1]
        self.r_cyc  = self.r[self.cycle_idx0:self.cycle_idxf+1]
        self.phase_cyc = self.phase[self.cycle_idx0:self.cycle_idxf+1]
        self.course_cyc = self.course[self.cycle_idx0:self.cycle_idxf+1]
    ...
    def investigate_course_computations(self):
        ...
        # -----------------------------
        crs_cart = np.arctan2(
            self.dy_cyc[self.ri_idx0:self.ri_idxf],
            self.dx_cyc[self.ri_idx0:self.ri_idxf]
        )
        crs_cart = np.mod(crs_cart, 2 * np.pi)
        ...
        crs_sph = -np.arctan2(az_dot * np.cos(self.el_ri), el_dot) + 2*np.pi
        crs_sph = np.mod(crs_sph, 2 * np.pi)

        # -----------------------------
        # Reference course (CSV)
        # -----------------------------
        crs_ref = np.mod(self.crs_ri, 2 * np.pi)
        ...
        ax.plot(crs_cart, label="Cartesian course", color="C0")
        ax.plot(crs_sph, label="Spherical course", color="C1")
        ax.plot(crs_ref, label="Reference (CSV)", color="C2")
    ...
    def compute_velocity_average_cart(self, C, U, u, start):
        """Compute average velocity over first or last k points of spline."""
        k = 10  # Number of points to average over
    ...
    def get_ri_ro_boundaries(self):
        ...
        ri_idxf = len(self.phase_cyc) - 1  # Last point of cycle

        self.ri_idx0, self.ri_idxf = ri_idx0, ri_idxf

        # Store points and velocities
        self.ri_p0_sph = np.array([self.az_cyc[ri_idx0], self.el_cyc[ri_idx0]])
        self.ri_pf_sph = np.array([self.az_cyc[ri_idxf], self.el_cyc[ri_idxf]])
        self.ri_p0 = np.array([self.x_cyc[ri_idx0], self.y_cyc[ri_idx0], self.z_cyc[ri_idx0]])
        self.ri_pf   = np.array([self.x_cyc[ri_idxf], self.y_cyc[ri_idxf], self.z_cyc[ri_idxf]])
        self.ri_v0 = np.array([self.dx_cyc[ri_idx0], self.dy_cyc[ri_idx0], self.dz_cyc[ri_idx0]])
        self.ri_vf   = np.array([self.dx_cyc[ri_idxf], self.dy_cyc[ri_idxf], self.dz_cyc[ri_idxf]])
        self.ri_c0 = self.course_cyc[ri_idx0]
        self.ri_cf   = self.course_cyc[ri_idxf]

        # Spherical ri segment
        self.az_ri = self.az_cyc[ri_idx0:ri_idxf+1]
        self.el_ri = self.el_cyc[ri_idx0:ri_idxf+1]
        self.r_ri  = self.r_cyc[ri_idx0:ri_idxf+1]
        self.crs_ri = self.course_cyc[ri_idx0:ri_idxf+1]

        # Cartesian ri segment
        self.x_ri, self.y_ri, self.z_ri = self.x_cyc[ri_idx0:ri_idxf+1], self.y_cyc[ri_idx0:ri_idxf+1], self.z_cyc[ri_idx0:ri_idxf+1]

        return (self.ri_p0, self.ri_pf,
                self.ri_v0, self.ri_vf,
                self.az_ri, self.el_ri, self.r_ri,
                self.ri_idx0, self.ri_idxf)
    ...
    def fit_spherical_spline(self, p=3, n_ctrl=8, course_penalty=1.0, eps_knot=1e-3):
        ...
        _, Nmat_sph, _ = self.evaluate_bspline(C0, p, U0, self.u_vals, return_basis=True)
        ...
        x0 = np.concatenate([C0[2:-2], du0])

        # -------------------
        # Bounds
        # -------------------
        lb_C = np.full_like(C0[2:-2], -3 * np.pi)
        ub_C = np.full_like(C0[2:-2],  3 * np.pi)
        ...
        def residuals(params):
            ...
            # Evaluate spline
            S_fit_sph, Nmat, _ = self.evaluate_bspline(C, p, U, self.u_vals, return_basis=True)
            ...
            res_course = np.array([self.crs_ri[0] - average_course_spline_start,
                           self.crs_ri[-1] - average_course_spline_end]) * course_penalty
    ...
    def fit_cartesian_spline(self, p=3, n_ctrl=8, vel_penalty=0.0, eps_knot=1e-3):
        ...
        C0 = np.vstack([self.ri_p0, C_inner_0, self.ri_pf])
        ...
        rhs = self.S_cart - (Nmat[:, [0, -1]] @ np.vstack([self.ri_p0,
                                                   self.ri_pf]))
        ...
        x0 = np.concatenate([C0[3:-3], du0])
        # x0 = C0[3:-3]  # Fix knots for now

        # -------------------
        # Bounds
        # -------------------
        lb_C = np.full_like(C0[3:-3], -1e4)
        ub_C = np.full_like(C0[3:-3], 1e4)
        ...
        def residuals(params):
            ...
            C = np.vstack([self.ri_p0, C, self.ri_pf])
            ...
            res_vel = np.array((vel_start - self.ri_v0,
                               vel_end   - self.ri_vf)).ravel() * vel_penalty
        ...
        C_opt = np.vstack([self.ri_p0, C_opt, self.ri_pf])
    ...
    def plot_spline_fit_cart(self):
        ...
        # Plot ri start/end
        if self.ri_p0 is not None and self.ri_pf is not None:
            ax.scatter(*self.ri_p0, color="green", s=30, label="ri start")
            ax.scatter(*self.ri_pf, color="red", s=30, label="ri end")
    ...
    def plot_spline_fit_sph(self):
        ...
        S_fit_sph = np.vstack([self.eval_spherical_spline(u) for u in self.u_vals])
...
# --- File paths ---
    ...
    ri_start, ri_end, ri_v0, ri_vf, az_ri, el_ri, r_ri, ri_idx0, ri_idxf = cycle.get_ri_ro_boundaries()

    # Fit a B-spline to the full cycle (or later to ri only)
    C_cart, u_vals, U = cycle.fit_cartesian_spline()
    print("Fitted Cartesian B-spline control points:\n", C_cart)
